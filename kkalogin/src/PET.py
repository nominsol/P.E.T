...
st.write("텍스트 파일을 업로드하고 해당 내용을 기반으로 객관식, 참/거짓 또는 서술형의 문제를 생성합니다.")
...
def main():
    if check_login(input_id):
        uploaded_file = st.file_uploader("텍스트 파일을 업로드하세요.", type=['txt', 'pdf', 'docx'])
        st.warning('텍스트가 한글 2000자 이상이라면 문제가 생성되지 않을 수 있습니다. 텍스트를 줄여주세요.', icon="⚠️")
        if uploaded_file:
            ...
            if file_extension == "txt":
                ...
                st.error("지원되지 않는 파일 형식입니다. TXT, PDF 또는 DOCX 파일을 업로드해주세요.")

            question_type = st.selectbox("생성할 문제 유형을 선택하세요.", ["객관식", "참/거짓형", "서술형"])

            if question_type == "객관식":
                num_options = st.number_input("각 문제에 대한 선택지 수를 선택하세요 :", min_value=2, max_value=10, value=4, step=1)
            else:
                num_options = 4

            num_questions = st.number_input("생성할 문제 수를 입력하세요 :", min_value=1, value=5)
        ...
        #     st.stop()
            ...
            if st.button("문제 생성"):
                st.session_state['questions'] = generate_questions_gpt35_turbo(text, num_questions, question_type,
                                                                           num_options)
                question_parts = st.session_state['questions'].replace("*", "\\*").split('Ans:')
            ...
            if st.button("질문 저장"):
                if st.session_state['Q']:
                    ...
                    st.markdown(get_binary_file_downloader_html(docx_file, '질문'), unsafe_allow_html=True)
                else:
                    st.write("문제가 생성되지 않았습니다. 먼저 '문제 생성' 버튼을 눌러 질문을 생성하세요.")

            if st.button("답 저장"):
                if st.session_state['A']:
                    docx_file = text_to_docx(st.session_state['A'])
                    st.markdown(get_binary_file_downloader_html(docx_file, '답'), unsafe_allow_html=True)
                else:
                    st.write("문제가 생성되지 않았습니다. 먼저 '문제 생성' 버튼을 눌러 질문을 생성하세요.")
